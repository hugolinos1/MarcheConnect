...
export default function AdminDashboard() {
  ...
  const handleAuth = async (e: React.FormEvent, forceRequest: boolean = false) => {
    ...
    try {
      ...
      if (isSigningUp || forceRequest) {
        // Tentative d'inscription ou demande d'accès
        try {
        ...
        if (forceRequest) {
          // L'utilisateur sollicite un code
          ...
          // L'utilisateur tente de s'inscrire avec un code existant
          const masterCode = currentConfig?.signupCode || "FELIX2026";
          if (signupCodeInput === masterCode) {
            // Seul Hugues peut écrire directement dans roles_admin via firestore.rules
            // Pour les autres, on crée une demande PENDING, Hugues validera
      ...
      if (err.code === 'auth/wrong-password') {
        ...
        setAuthError("Email non trouvé.");
      } else {
        setAuthError("Erreur d'authentification: " + (err.message || "Veuillez réessayer."));
      }
    } finally {
      setIsAuthLoading(false);
    }
  ...
  const handleUnlockWithCode = () => {
    ...
    if (inputActivationCode === masterCode || (personalCode && inputActivationCode === personalCode)) {
      // NOTE: Seul l'Admin peut réellement écrire dans roles_admin. 
      // Si le code est bon, on affiche un message encourageant mais Hugues doit cliquer sur Valider.
