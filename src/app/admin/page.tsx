...
export default function AdminDashboard() {
  ...
  // Sorting exhibitors by date DESC client-side to avoid index issues
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <Tabs defaultValue="exhibitors">
          ...
          <TabsContent value="settings" className="space-y-8">
            <Card className="max-w-4xl mx-auto border-t-4 border-t-primary bg-white shadow-xl">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="p-4 bg-muted/30 rounded-xl space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-2"><label className="text-xs font-bold">Année du Marché</label><Input type="number" value={configForm.marketYear} onChange={(e) => setConfigForm({...configForm, marketYear: parseInt(e.target.value)})} /></div>
                    <div className="space-y-2"><label className="text-xs font-bold">Nom de l'Édition</label><Input value={configForm.editionNumber} onChange={(e) => setConfigForm({...configForm, editionNumber: e.target.value})} placeholder="Ex: 6ème édition" /></div>
                    <div className="space-y-2 md:col-span-2"><label className="text-xs font-bold">URL de l'Affiche (Hébergée)</label><Input value={configForm.posterImageUrl} onChange={(e) => setConfigForm({...configForm, posterImageUrl: e.target.value})} placeholder="https://..." /></div>
                  </div>
                </div>
                ...
                <Card className="border-t-4 border-t-amber-500 shadow-md">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {(!adminRequests || adminRequests.filter(r => r.status === 'PENDING').length === 0) && (
                      <div className="text-center py-8 text-muted-foreground text-sm italic">Aucune demande en attente</div>
                    )}
                  </CardContent>
                </Card>
                <Card className="border-t-4 border-t-primary shadow-md">
                  <CardHeader><CardTitle className="text-lg flex items-center gap-2 text-primary"><ShieldCheck className="w-5 h-5" /> Équipe Administrateur</CardTitle></CardHeader>
                  <CardContent className="space-y-4">
